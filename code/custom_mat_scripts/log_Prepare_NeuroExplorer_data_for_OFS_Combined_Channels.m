...
%    disp(nex5FilePath)
    [~, baseFolderName, ~] = fileparts(filePath);
    
    % Build the full path for the nex5 file including the base folder name
    nex5FilePath = fullfile(filePath, ['Chan_' num2str(min(channel_num{part})) '-' num2str(max(channel_num{part})) '_' baseFolderName '_v2.nex5']);
